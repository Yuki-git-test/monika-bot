...
from .trophy_update_leaderboard import (
    new_public_first_place_announcement,
    trophy_update_public_leaderboard_func,
)
...
# 🍭──────────────────────────────
async def public_trophy_remove_func(
    bot: commands.Bot,
    ...
    current_trophies_info = await fetch_user_public_trophies(bot, member)
    ...
    await trophy_update_public_leaderboard_func(bot, guild)

    # Build embed
    embed = discord.Embed(
        title="Public Trophies Removed",
        description=(
    ...
    old_first_place_info = await fetch_current_public_leaderboard_info(bot)
    old_first_place_id = (
        old_first_place_info.get("first_place_id") if old_first_place_info else None
    )
    current_first_place = await get_public_first_place(bot)
    if current_first_place:
        ...
        if current_first_place_id != old_first_place_id:
            await new_public_first_place_announcement(
                bot=bot,
                guild=guild,
