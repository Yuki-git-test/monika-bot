...
from .trophy_update_leaderboard import trophy_update_public_leaderboard_func
...
# 🍭──────────────────────────────
async def public_trophy_multi_func(
    bot: commands.Bot,
    ...
    for member in members:
        ...
        if not member_info:
            ...
            desc_line = (
                f"{member.mention} - {member.display_name} skipped (no trophy data)."
            )
    ...
    await update_public_leaderboard_first_place(bot)
